...
public:
    ...
    float moveTo(float position_mm, float speed_mm_s, float accell_mm_s2) override {
        ...
        if (position_mm > this->max_position_mm_) {
            final_position_mm = this->max_position_mm_;
        } else if (position_mm < this->min_position_mm_) {
            final_position_mm = this->min_position_mm_;
        }

        float distance = final_position_mm - current_position_mm_;
